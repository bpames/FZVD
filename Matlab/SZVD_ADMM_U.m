function [x,y,z,its, errtol] = SZVD_ADMM_U(R, N, RN, D, sols0,gamma, alpha, beta, pentype, Q,  tol, maxits, quiet)
...
for iter=1:maxits    
    ...
    if isequal(pentype, 'diag')
        % Save previous iterate.
        yold = y;
        
        
        % Calculate vector input for shrinkage.
        b = Dx(N*x) + z;
        
        % Update y using multiplicative shrinkage.
        s = vec_shrink(b, gamma*alpha);
        y = s./(gamma*(1-alpha)*diag(Q) + beta); % Normalize y (if necessary).
