...
for i=1:length(p)
    ...
    for j=1:T
        ...
        times(j, i, meth) =toc(btime); % Stop timer after training is finished.        
        ...
        times(j,i, meth) =toc(stime); % Stop timer after training is finished.        
        % Check classification and feature selection performance.
        [stats,~,~,~]=predict(DVs,test,classMeans);
        errs(j,i, meth)=stats.mc;
        feats(j,i, meth)=sum(stats.l0);
...
%         gamma
        ...
        times(j,i, meth) =toc(szvd01start);
