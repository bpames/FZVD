function [bestDVs, bestind, inds, bestgamma,  cv_scores, classMeans] = PenZDAcv(train, nfolds, D, gmults, consttype, sparsity_level, beta, tol, maxits,quiet)
...
[bestscore,bestind] = min(meanscores);
inds = find(meanscores == bestscore);
%  bestind = floor(median(inds));
%bestind = min(inds);
